...
class DefinedNamesView(ft.Column):
    ...
    def _display_results(self, results: dict):
        ...
        summary = ft.Row(
            controls=[
                status_badge(f"{counts['coincidencias']} coincidencias", t.SUCCESS, ft.Icons.CHECK_CIRCLE_OUTLINE),
                status_badge(f"{counts['diferencias']} diferencias", t.ERROR, ft.Icons.WARNING_AMBER_ROUNDED),
                status_badge(f"{counts['solo_en_archivo1']} solo en base", t.WARNING, ft.Icons.ARROW_BACK),
                status_badge(f"{counts['solo_en_archivo2']} solo en comparado", t.WARNING, ft.Icons.ARROW_FORWARD),
            ],
            wrap=True,
            spacing=8,
            run_spacing=8,
        ...
        if only1:
            controls.append(self._build_only_section(
        ...
        if matches:
            controls.append(self._build_only_section("Coincidencias", matches, t.SUCCESS))
    ...
    def _build_diff_section(self, title: str, items: list, color: str) -> ft.Column:
        ...
        for item in items[:200]:
            ...
            rows.append(
                ft.DataRow(cells=[
                    ft.DataCell(ft.Text(name, size=11, weight=ft.FontWeight.W_600, selectable=True)),
        ...
        return ft.Column(
            controls=[
                ft.Text(title, weight=ft.FontWeight.W_600, size=t.BODY_SIZE, color=color),
                ft.Container(
                    content=ft.DataTable(
                        columns=[
                            ft.DataColumn(ft.Text("Nombre", weight=ft.FontWeight.W_600, size=t.CAPTION_SIZE)),
                            ft.DataColumn(ft.Text(file1_stem, weight=ft.FontWeight.W_600, size=t.CAPTION_SIZE)),
                            ft.DataColumn(ft.Text(file2_stem, weight=ft.FontWeight.W_600, size=t.CAPTION_SIZE)),
                        ],
                        rows=rows,
    ...
    def _build_only_section(self, title: str, items: list, color: str) -> ft.Column:
        ...
        for item in items[:200]:
            rows.append(
                ft.DataRow(cells=[
                    ft.DataCell(ft.Text(item.get("nombre", ""), size=11, weight=ft.FontWeight.W_600, selectable=True)),
                    ft.DataCell(ft.Text(str(item.get("valor", "")), size=11, selectable=True)),
                ])
            )
            ...
            controls=[
                ...
                ft.Container(
                    content=ft.DataTable(
                        columns=[
                            ft.DataColumn(ft.Text("Nombre", weight=ft.FontWeight.W_600, size=t.CAPTION_SIZE)),
                            ft.DataColumn(ft.Text("Valor", weight=ft.FontWeight.W_600, size=t.CAPTION_SIZE)),
                        ],
                        rows=rows,
                        border=ft.border.all(1, t.BORDER),
                        border_radius=t.BUTTON_RADIUS,
